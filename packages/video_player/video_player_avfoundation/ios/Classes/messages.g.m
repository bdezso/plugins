...
@interface FLTPausePointsMessage ()
...
- (nullable id)readValueOfType:(UInt8)type 
{
  switch (type) {
    ...
    case 131:     
      return [FLTPausePointsMessage fromMap:[self readValue]];
...
- (void)writeValue:(id)value 
{
  ...
  if ([value isKindOfClass:[FLTPausePointsMessage class]]) {
    [self writeByte:131];
    [self writeValue:[value toMap]];
  } else 
...
void FLTAVFoundationVideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLTAVFoundationVideoPlayerApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.AVFoundationVideoPlayerApi.setPausePoints"
        binaryMessenger:binaryMessenger
        codec:FLTAVFoundationVideoPlayerApiGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setPausePoints:error:)], @"FLTAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(setPausePoints:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTPausePointsMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api setPausePoints:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    }
    else {
...
@interface FLTVideoPlayerFlutterApiCodecReader : FlutterStandardReader
@end
@implementation FLTVideoPlayerFlutterApiCodecReader
- (nullable id)readValueOfType:(UInt8)type 
{
  switch (type) {
    case 128:     
...
@interface FLTVideoPlayerFlutterApiCodecWriter : FlutterStandardWriter
@end
@implementation FLTVideoPlayerFlutterApiCodecWriter
- (void)writeValue:(id)value 
{
  if ([value isKindOfClass:[FLTPositionMessage class]]) {
    [self writeByte:128];
...
@interface FLTVideoPlayerFlutterApiCodecReaderWriter : FlutterStandardReaderWriter
@end
@implementation FLTVideoPlayerFlutterApiCodecReaderWriter
- (FlutterStandardWriter *)writerWithData:(NSMutableData *)data {
  return [[FLTVideoPlayerFlutterApiCodecWriter alloc] initWithData:data];
}
- (FlutterStandardReader *)readerWithData:(NSData *)data {
  return [[FLTVideoPlayerFlutterApiCodecReader alloc] initWithData:data];
}
@end

NSObject<FlutterMessageCodec> *FLTVideoPlayerFlutterApiGetCodec() {
  static dispatch_once_t sPred = 0;
  static FlutterStandardMessageCodec *sSharedObject = nil;
  dispatch_once(&sPred, ^{
    FLTVideoPlayerFlutterApiCodecReaderWriter *readerWriter = [[FLTVideoPlayerFlutterApiCodecReaderWriter alloc] init];
    sSharedObject = [FlutterStandardMessageCodec codecWithReaderWriter:readerWriter];
  });
  return sSharedObject;
}
...
@interface FLTVideoPlayerFlutterApi ()
@property (nonatomic, strong) NSObject<FlutterBinaryMessenger> *binaryMessenger;
@end

@implementation FLTVideoPlayerFlutterApi
...
- (void)autoPauseHappenMsg:(FLTPositionMessage *)arg_msg completion:(void(^)(NSError *_Nullable))completion {
  FlutterBasicMessageChannel *channel =
    [FlutterBasicMessageChannel
      messageChannelWithName:@"dev.flutter.pigeon.VideoPlayerFlutterApi.autoPauseHappen"
      binaryMessenger:self.binaryMessenger
      codec:FLTVideoPlayerFlutterApiGetCodec()];
